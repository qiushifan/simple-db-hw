...
public class HeapPage implements Page {
    ...
    private int getNumTuples() {
        double res = (double) (BufferPool.getPageSize() * 8) / (double) (td.getSize() * 8 + 1);
    ...
    private int getHeaderSize() {
        int tmp = getNumTuples();
        double res = (double) tmp / 8;
    ...
    public boolean isSlotUsed(int i) {
        int index = (int) Math.floor((double) i / 8);
        int innerIndex = i % 8;
        byte tmp = this.header[index];
    ...
    public HeapPageId getNextPage(int totalNum) {
        int currentNum = this.pid.getPageNumber();
        if (currentNum == totalNum - 1) {
            return null;
        } else {
            return new HeapPageId(this.pid.getTableId(), this.pid.getPageNumber() + 1);
        }
    }
}
